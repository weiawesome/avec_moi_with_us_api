...
type HandlerEditInfo struct {
	Service    user.ServiceEditInformation
	LogService logger.ServiceLogger
}

func (h *HandlerEditInfo) Handle(c *gin.Context) {
	...
	if stringDataMail, ok := mail.(string); ok {
		if jsonData, ok := data.(userRequest.EditInformation); ok {
