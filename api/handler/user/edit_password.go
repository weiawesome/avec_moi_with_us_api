...
type HandlerEditPassword struct {
	Service    user.ServiceEditPassword
	LogService logger.ServiceLogger
}

func (h *HandlerEditPassword) Handle(c *gin.Context) {
	...
	if stringDataMail, ok := mail.(string); ok {
		if jsonData, ok := data.(userRequest.EditPassword); ok {
